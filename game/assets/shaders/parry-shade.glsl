...
const float gradientFalloff = 0.035;        // falloff distance outside rectangle bounds (in texture space, 0.02 = 2% of screen)
const float gradientPower = 0.25;           // gradient intensity curve (higher = stronger gradient, sharper falloff at end; 1.0 = linear)
const float edgeSmoothness = 1.0;          // smoothness of band transitions (0.0 = sharp, 1.0 = very smooth)
...
void effect()
{
    ...
    for (int i = 0; i < gradientCount; i++) {
        ...
        // Quantize falloff to match gradient steps with smooth edges
        if (gradientSteps > 1.0 && falloffMultiplier < 1.0) {
            float scaled = falloffMultiplier * gradientSteps;
            ...
            falloffMultiplier = (stepFloor + smoothedFrac) / (gradientSteps - 1.0);
        }
        ...
        // Draw solid white outline on edge pixels (only inside rectangle)
